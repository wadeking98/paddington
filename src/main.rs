...
use crate::{crypt::{decrypt::padding_oracle_decrypt, detector::Oracle, forge::padding_oracle_forge}, helper::Config};
...
impl HTTPDoubleOracle{
    fn new(url: String, headers:Vec<(String, String)>, method: Method, data: Option<String>, encoding: Vec<Encoding>, params: Vec<String>, search_pat: Option<String>, proxy: Option<String>) -> Self{
        let oracle = HTTPOracle::new(url, headers, method, data, encoding, params, search_pat, proxy);
        let base_ct = oracle.base_ct.clone();
        Self { single_oracle: oracle, base_ct }
    }
}
    ...
    async fn exec(&self, ct: &[u8]) -> String{
        ...
        for enc in self.encoding.clone().iter().rev(){
            res = match enc{
                 ...
                 Encoding::URL => encode(&String::from_utf8(res).expect("Error: invalid string when url encoding")).as_bytes().to_vec()
            };
        }
        ...
        let client = client_builder.build().expect("Error: could not build request");
        ...
        let response_body = response.text().await.expect("Error: could not convert response body to text");
...
fn search_json_obj(val: &mut Value, search_key: String, replace_opt: Option<String>) -> Option<String>{
    ...
    for p in oracle.params.clone(){
        ...
        for query_param in url.as_pairs(){
            if p.eq(query_param.0){
                ...
                oracle.url = oracle.url.replace(&query_param.1.to_string(), &injection_point);
            }
        }
        ...
        if let Some(ref mut body_data) = oracle.data{
            ...
            if let Ok(mut json_data) = json_data_res{
                if let Some(success_res) = search_json_obj(&mut json_data, p.clone(), Some(injection_point.clone())){
...
fn decode_ct(ct: String, encoding: Vec<Encoding>) -> Vec<u8>{
    ...
    for enc in encoding{
        res = match enc{
            Encoding::HEX => hex::decode(String::from_utf8(res).expect("Error: invalid string")).expect("Error: invalid hex string"),
            Encoding::B64 => BASE64_STANDARD.decode(String::from_utf8(res).expect("Error: invalid string")).expect("Error: invalid b64 string"),
            Encoding::B64Url => BASE64_URL_SAFE.decode(String::from_utf8(res).expect("Error: invalid string")).expect("Error: invalid b64 url string"),
            Encoding::URL => decode(&String::from_utf8(res).expect("Error: invalid string when url decoding")).expect("Error: invalid encoded url string").as_bytes().to_vec()
        };
    }
    return res;
}
...
impl HTTPOracle{
    fn new(url: String, headers:Vec<(String, String)>, method: Method, data: Option<String>, encoding: Vec<Encoding>, params: Vec<String>, search_pat: Option<String>, proxy:Option<String>) -> Self{
        ...
        if let Some(search_pat) = search_pat{
            let re = RegexBuilder::new(&search_pat).multi_line(true).build().expect(&("Error: Failed to compile regex ".to_owned()+&search_pat));
            pat = Some(re);
        }
        let mut prox = None;
        if let Some(p) = proxy{
        ...
        let mut oracle = Self { url, headers, method, data, encoding, params, search_pat:pat , base_ct: vec![], proxy:prox};
...
async fn main() {
    ...
    let standard_oracle = Box::new(HTTPOracle::new(args.url.clone(), headers.clone(), args.method.clone().unwrap_or(Method::GET),args.data.clone(),encoding.clone(),args.params.clone(), args.search_pat.clone(), args.proxy.clone()));
    let standard_ct = standard_oracle.base_ct.clone();
    let double_oracle = Box::new(HTTPDoubleOracle::new(args.url,headers, args.method.unwrap_or(Method::GET),args.data,encoding.clone(),args.params, args.search_pat, args.proxy));
    ...
    for (oracle, base_ct, log) in oracles{
        ...
        if let Some(forge_string) = args.forge.clone(){
            let result = padding_oracle_forge(forge_string.as_bytes(), &ct.clone(), oracle, config.clone()).await;
            if let Ok(ct) = result{
